...
},
{
	type:'input',
	name:'sleep',
	message:'[>] Uyku Süresi (MiliSeconds):',
	validate: function(value){
		value = value.match(/[0-9]/);
		if (value) return true;
...
const Excute = async function(User,sleep,mysyntx){
	try {
		console.log(chalk`\n{yellow [?] Giriş Yapılıyor . . .}`);
		const doLogin = await Login(User);
		console.log(chalk`{green [!] Giriş Başarılı}, {yellow [?] Takipçiler kontrol ediliyor . . .}`);
		const task = [
		...
		console.log(chalk`{bold.green  | Takipçiler : ${getFollowers.length}\n | Takip Ettiğin : ${getFollowing.length}}`);
		...
		console.log(chalk`{blue  | Takipten Çıkarılacak Hesap : ${AccountToUnfollow.length}}`)
...
console.log(chalk`
  ...
  [?] {bold.green Furkan Gündoğdu *SET SLEEP!}

  ——————————————————  [SAYGILARIMLA]  ————————————————————
  [✓] CODE BY Furkan Gündoğdu (mrfurkangundogdu@gmail.com)
  [✓] @furkan.gundogdu06  | mrfurkangundogdu@gmail.com 
  —————————————————————————————————————————————————————
  Ne var Ne Yok?
  1. Giriş Yapın (ITTYW)
  —————————————————————————————————————————————————————}
